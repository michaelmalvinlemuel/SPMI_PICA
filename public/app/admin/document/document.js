(function () {
	angular
		.module('app')
		.controller('DocumentController', ['$scope', '$state', '$timeout', 'StandardService', DocumentController])
		.directive('documentTree', ['$timeout', DocumentTree])
})()
...
function DocumentController ($scope, $state, $timeout, StandardService) {
	
	$scope.pdfUrl = '';//'/upload/standardDocument/STANDARDKOMPETENSIKELULUSAN_20151026110540.pdf'
	...
	$scope.convert = function() {
		for(var i = 0 ; i < $scope.documents.length ; i++) {
			...
			var standard = $scope.documents[i]
			...
			for(var j = 0 ; j < standard.standard_documents.length ; j++) {
				...
				for(var k = 0 ; k < standard_document.guides.length ; k++) {
					...
					for(var l = 0 ; l < guide.instructions.length ; l++) {
						...
						for(var m = 0 ; m < instruction.forms.length ; m++) {
							...
							forms.push({
								...
								onSelect: function(branch) {
									$scope.pdfUrl = 'upload/form/' + branch.document
								},
						...
						instructions.push({
							...
							onSelect: function(branch) {
								$scope.pdfUrl = 'upload/instruction/' + branch.document
							},
					...
					guidelines.push({
						...
						onSelect: function(branch) {
							$scope.pdfUrl = 'upload/guide/' + branch.document
						},
				...
				standard_documents.push({
					...
					onSelect: function(branch) {
						$scope.pdfUrl = 'upload/standardDocument/' + branch.document
					},
