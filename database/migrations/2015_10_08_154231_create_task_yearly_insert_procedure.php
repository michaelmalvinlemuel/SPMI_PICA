...
class CreateTaskYearlyInsertProcedure extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::unprepared("CREATE PROCEDURE task_yearly_insert_procedure (
                workId INT
            )
            ...
            BEGIN
                ...
                SELECT schedule_yearlies.date_start, schedule_yearlies.month_start
                INTO dateStart, monthStart
                FROM schedule_yearlies
                INNER JOIN works 
                    ON (schedule_yearlies.id = works.schedule_id)
                WHERE (works.id = workId) AND (schedule_yearlies.deleted_at IS NULL);
                    
                        IF (CURRENT_TIMESTAMP > expiredAt OR batchTotal = 0) THEN
                            SET newExpiredAt = next_year(CURRENT_TIMESTAMP, dateStart, monthStart);
    ...
    {
        DB::unprepared("DROP PROCEDURE IF EXISTS task_yearly_insert_procedure;");
    }
}
