...
class CreateTaskWeeklyInsertProcedure extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::unprepared("CREATE PROCEDURE task_weekly_insert_procedure (
                workId INT
            )
            ...
            BEGIN
                ...
                SET dayStart = (
                    SELECT schedule_weeklies.day_start
                    FROM schedule_weeklies
                    INNER JOIN works 
                        ON (schedule_weeklies.id = works.schedule_id)
                    WHERE works.id = workId AND schedule_weeklies.deleted_at IS NULL);
                
                IF (CURRENT_TIMESTAMP > expiredAt OR batchTotal = 0) THEN
                    SET newExpiredAt = next_weekday(CURRENT_TIMESTAMP, dayStart);
